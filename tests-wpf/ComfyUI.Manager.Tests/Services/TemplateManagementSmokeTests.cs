...
/// v1.0.0.x (2026-09-02) T31: Whisper (openai/whisper 纯 CLI,无 Web UI) + LTXVideo
/// (Lightricks/LTX-Video run-ltx2-distilled.bat 纯 CLI,无 Web UI) 已下线 —
/// 用户决策"删除所有不带 Web UI 的",从 ClonedBuiltinKinds + AllBuiltins 移除,
/// 总数 8 → 6。ENVTemplate/Whisper + ENVTemplate/LTXVideo 目录也待删(T31d)。
/// v1.0.0.x: A1111 不再是内置模板(已下线),从所有列表移除,总数 6 → 5。
/// </summary>
public sealed class TemplateManagementSmokeTests
{
    ...
    /// v1.0.0.x (2026-09-02) T31: Whisper + LTXVideo 已下线 (8 → 6 built-ins),
    /// 同时 ENVTemplate/Whisper + ENVTemplate/LTXVideo 目录也应清掉。
    ...
    /// v1.0.0.x (2026-09-02) T31: Whisper + LTXVideo 已下线,8 → 6。
    /// </summary>
    private static IEnumerable<(string Kind, TemplateConfig Cfg)> AllBuiltins()
    {
        ...
        // v1.0.0.x (2026-09-02) T31:Whisper + LTXVideo 纯 CLI 已下线,从 AllBuiltins 移除。
    ...
    /// - GitHub 类(OpenVoice + HunyuanVideo/CogVideoX +
    ...
    /// v1.0.0.x (2026-09-02) T31: Whisper + LTXVideo 已下线,从 GitHub case 列表移除。
    /// </summary>
    [Fact]
    ...
    private static TemplateConfig BuildCfg(string kind) => kind switch
    {
        ...
        // v1.0.0.x (2026-09-02) T31:Whisper + LTXVideo 已下线,factory 删,switch case 删。
