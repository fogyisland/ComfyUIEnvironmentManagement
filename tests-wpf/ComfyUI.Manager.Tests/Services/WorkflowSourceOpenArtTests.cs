...
public class WorkflowSourceOpenArtTests
{
    ...
    public async Task SearchAsync_ItemsShape_ParsesEntries()
    {
        ...
        var src = new OpenArtSource(MockHttp(json));

        var result = await src.SearchAsync(query: "", maxResults: 10);

        Assert.Single(result);
        Assert.Equal("abc", result[0].SourceId);
        Assert.Equal(WorkflowSourceKind.OpenArt, result[0].Source);
    ...
    {
        var src = new OpenArtSource(MockHttp("server error", HttpStatusCode.InternalServerError));
    ...
    [Fact(Skip = "Integration: hits real OpenArt endpoint")]
    public async Task LiveFetch_OpenArt_RealEndpoint_ReturnsEntries()
    {
        var src = new OpenArtSource(new HttpClient());
        var result = await src.SearchAsync(query: "", maxResults: 10);
